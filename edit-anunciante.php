...
if ($_POST['editar'] == '1') {
    $editUser = (new BD())->editAnunciante($id, $celular, $email, $nome, $cep, $saldo);
...
if ((!isset($_SESSION['user']) == true) and (!isset($_SESSION['pass']) == true)) {
    ...
    $anunciantes = (new BD())->query("SELECT * FROM anunciantes a WHERE id = {$anuncianteId}");
}
...
<main class="container">
    <a class="btn__default formRegister__itemButton" style="text-decoration: none;" href="anunciantes.php">Anunciantes</a>
    <br>
    <form class="container__formRegister" method="POST" action="">
        ...
        <label class="formRegister__item" for="name">
            Nome
            <input class="formRegister__itemInput" type="text" name="nome" value="<?=$anunciantes[0]['nome']?>">
        </label>
        <label class="formRegister__item" for="email">
            email
            <input class="formRegister__itemInput" type="email" name="email" value="<?=$anunciantes[0]['email']?>">
        </label>
        <label class="formRegister__item" for="phone">
            celular
            <input class="formRegister__itemInput" type="number" maxlength="11" name="celular" value="<?=$anunciantes[0]['celular']?>">
        </label>
        <label class="formRegister__item" for="cep">
            cep
            <input class="formRegister__itemInput" type="text" maxlength="8" name="cep" value="<?=$anunciantes[0]['cep']?>">
        </label>
        <label class="formRegister__item" for="cep">
            saldo
            <input class="formRegister__itemInput" type="text" name="saldo" value="<?=$anunciantes[0]['saldo']?>">
        </label>
        <button class="formRegister__itemButton btn__default">Editar</button>

        <input type="hidden" name="editar" value="1">
        <input type="hidden" name="id" value="<?=$anunciantes[0]['id']?>">
